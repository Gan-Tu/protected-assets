  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <Button
        type="button"
        variant="outline"
        className="h-10 text-xs font-bold uppercase tracking-wider border-zinc-200 hover:bg-zinc-50 transition-colors"
        onClick={() => handleOAuth("google")}
        disabled={isPending}
      >
        {isPending ? <LoaderCircleIcon className="size-3 animate-spin" /> : <GoogleGlyph />}
        Google
      </Button>
      <Button
        type="button"
